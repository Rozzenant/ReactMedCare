...
const First_aid_Filter: React.FC<FilterProps> = ({ first_aids, onFilterChange }) => {
  const [filter, setFilter] = useState<string>('');
  const [priceFrom, setPriceFrom] = useState<string>('');
  const [priceTo, setPriceTo] = useState<string>('');

  const FilterChange = () => {
    const filteredFirst_aid = first_aids.filter((first_aid) =>
  ...
  React.useEffect(() => {
    FilterChange();
  }, [filter, priceFrom, priceTo, first_aids, onFilterChange]);

  return (
    <div style={{ display: 'flex', justifyContent: 'center', gap: '5px' }} >
      <input type='text' placeholder='Поиск по названию' value={filter} onChange={Filter} />
      <input type='number' placeholder='От' id='priceFrom' value={priceFrom} onChange={PriceFrom} />
      <input type='number' placeholder='До' id='priceTo' value={priceTo} onChange={PriceTo} />
    </div>
  );
};
