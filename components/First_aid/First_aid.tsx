...
const First_aid: React.FC<First_aid_Props> = ({ first_aid }) => {
  ...
  const ButtonClick = () => {
    navigate(`/${first_aid.First_aid_ID}`, { state: { object: first_aid, returnTo: location.pathname } });
  };

  return (
    <Card style={{ width: '267px', height: '100%', marginTop: '5px', marginLeft: "75px" }}>
      <Card.Img style={{ width: '267px', height: '200px', objectFit: "cover" }} variant="top" src={first_aid.Image_URL || defaultImg} />
      <Card.Body>
        <Card.Title style={{ height: "3rem" }}>{first_aid.First_aid_Name}</Card.Title>
        <Card.Text>
          {first_aid.Price} $
        </Card.Text>
